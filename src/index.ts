...
import CompilerStream from './CompilerStream';
import TemplatesRuntimeI18nTransformer from './TemplatesRuntimeI18nTransformer';
...
const Compiler:{[key:string]:string} = {
...
export interface ITemplatesI18nOptions {
    /**
...
export function templates(args:string[], i18nOptions?:ITemplatesI18nOptions):CompilerStream {
    const enableRuntimeI18n = i18nOptions ? async (stdout:string) => {
        ...
        let fileNames = <string[]>(await fg(i18nOptions.inputGlob));
...
import DepsSorter, { IEntryPoint } from './DepsSorter';

export interface ICompilerEntryPoint extends Partial<IEntryPoint> {
    /**
...
export async function compiler(baseArgs:string[], sourceGlob:string|string[], entryPoints:ICompilerEntryPoint[]) {
    ...
    let deps = sorter.getDeps(
        entryPoints.map(entryPoint => {
            let { moduleId, extraSources } = entryPoint;
            extraSources = extraSources || [];
            return { moduleId, extraSources };
        })
    );
    ...
    let depsFlag = deps.map((depsOfAModule, index) => {
        ...
        return [
            '--module', `${entryPoint.moduleName}:${depsOfAModule.length}:${(entryPoint.deps || []).join(':')}`,
